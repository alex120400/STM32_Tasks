...
namespace Antmicro.Renode.Peripherals.Timers
{
    ...
    public class STM32F3_Timer : LimitTimer, IDoubleWordPeripheral, IKnownSize, INumberedGPIOOutput, IPeripheralRegister<IGPIOReceiver, NumberRegistrationPoint<int>>, IPeripheralRegister<IGPIOReceiver, NullRegistrationPoint>
    {
        public STM32F3_Timer(IMachine machine, long frequency, uint initialLimit) : base(machine.ClockSource, frequency, limit: initialLimit, direction: Direction.Ascending, enabled: false, autoUpdate: false)
        {
            ...
            LimitReached += delegate
            {
                ...
                if(repetitionsLeft == 0) // changed from original: if(updateInterruptEnable.Value && repetitionsLeft == 0)
                {
                    ...
                    this.Log(LogLevel.Noisy, "Update event occurred");
                    updateInterruptFlag = true;
                    if(updateInterruptEnable.Value)
                    {
                        this.Log(LogLevel.Noisy, "Update interrupt pending");
                        UpdateInterrupts();
                    }
                }
